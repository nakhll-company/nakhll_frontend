        ...
        <div className={styles.main_wrapper}>
                ...
                <form id="myform" onSubmit={(event) => {
                    ...
                    <div className={styles.inputs_wrapper}>
                        ...
                        <div className={styles.input_file_wrapper}>
                            ...
                            <input className={styles.inputs_file} type="file" id="productExcelUpload" name="productExcelUpload" accept=".csv"
                                onChange={(event) => {
                                    ...
                                    previewFileCsv(event, event.target.files[0], dispatch, getGroupProductCsvData, getGroupProductCsvHeader);
                                }}
                            />
                            {file && <div className={styles.preview_wrapper}>
                                <span>مشخصات فایل</span><br />
                                <span>نام فایل : {file.name}</span><br />
                                <span>حجم فایل : {calculateFileSize(file)}</span><br />
                            </div>}
                            <label className={styles.inputs_label} htmlFor="productExcelUpload">
                                <div className={styles.add_image_container}>
                                    ...
                                    <p style={{ fontSize: "15px" }} className="mt-2">افزودن فایل csv</p>
                                </div>
                            </label>
                        </div>
                        {/* input zip */}
                        <div className={styles.input_file_wrapper}>
                            ...
                            <input className={styles.inputs_file} type="file" id="productZipFile" name="productZipFile" accept=".zip,.rar,.7zip" onChange={(event) => {
                                setZipFile(event.target.files[0]);
                            }} />
                            {zipFile && <div className={styles.preview_wrapper}>
                                <span>مشخصات فایل</span><br />
                                <span>نام فایل : {zipFile.name}</span><br />
                                <span>حجم فایل : {calculateFileSize(zipFile)}</span><br />
                            </div>}
                            <label className={styles.inputs_label} htmlFor="productZipFile">
                                <div className={styles.add_image_container}>
                                    <i style={{ fontSize: "25px" }}>+</i>
                                    <p style={{ fontSize: "15px" }} className="mt-2">افزودن فایل zip</p>
                                </div>
                            </label>
                        </div>
                    </div>
            ...
            {false && file && zipFile &&
                <>
                    ...
                    <div>
                        ...
                        <p>لطفا اسم ستون های فایل بارگذاری شده را براساس اسم هایی که در سلکت باکس موجود می باشند تغییر دهید</p>
                        <p>ستون هایی با نام عنوان، بارکد، موجودی، قیمت جدید، قیمت قدیم، دسته بندی، عکس اصلی باید حتما در فایل بارگذاری شده موجود باشند</p>
                        {groupProductCsvHeader.map((column, index) => {
                            return (
                                <select key={index} className={styles.select_box_colmuns} onChange={(event) => {
                                    changeCsvColumns(event, groupProductCsvData, setUpdateCsv, column, setNextStep);
                                }}>
            ...
            {false && nextStep && <>
                ...
                <div>
                    ...
                    <p>در صورتی که محصول مورد نظر تخفیف ندارد لطفا ستون قیمت قدیم را با مقدار صفر پرنمایید و قیمت محصول را در ستون قیمت جدید بنویسید</p>
                    <table className={styles.table}>
                        <thead>
                            <tr className={styles.tr} key={"header"}>
                                {updateCsv.length > 0 && Object.keys(updateCsv[0]).map((value, index) => {
                                    switch (value) {
                                        case "Title":
                                            return <th className={styles.th} key={index}>عنوان</th>
                                        case "barcode":
                                            return <th className={styles.th} key={index}>بارکد</th>
                                        case "Inventory":
                                            return <th className={styles.th} key={index}>موجودی</th>
                                        case "Price":
                                            return <th className={styles.th} key={index}>قیمت جدید</th>
                                        case "OldPrice":
                                            return <th className={styles.th} key={index}>قیمت قدیم</th>
                                        case "Net_Weight":
                                            return <th className={styles.th} key={index}>وزن</th>
                                        case "Weight_With_Packing":
                                            return <th className={styles.th} key={index}>وزن با بسته بندی</th>
                                        case "category_id":
                                            return <th className={styles.th} key={index}>دسته بندی</th>
                                        case "image_1":
                                            return <th className={styles.th} key={index}>عکس اصلی</th>
                                        case "image_2":
                                            return <th className={styles.th} key={index}>عکس دوم</th>
                                        case "image_3":
                                            return <th className={styles.th} key={index}>عکس سوم</th>
                                    }
                                }
                                )}
                        ...
                        <tbody>
                            {updateCsv.length > 0 && updateCsv.map((item, index) => (
                                <tr className={styles.tr} key={Math.random()}>
                                    {Object.keys(item).map((key) => (
                                        <td className={`${styles.td} ${tableValidition(key, item[key]) ? styles.error : ""}`} key={Math.random() * 0.1}>
                                            <input type="text" defaultValue={item[key]} className={styles.readOnly_input} readOnly={true} onDoubleClick={(event) => {
