...
const MobileList = ({ loading, productList, activeHojreh, getProduct }) => {
    ...
    return (
        <div className={styles.wrapper}>
                        ...
                        <Link href={`/fp/product/update/product/${value.ID}`} key={index} passHref>
                            <div className={`${styles.product_card}`}>
                                ...
                                <div className={styles.second_row}>
                                    ...
                                    {value.OldPrice === 0 && (
                                        <CustomLabel type="normal" value={`${value.Price / 10}تومان`} label="قیمت" />
                                    )}
                                    {value.OldPrice !== 0 && (
                                        <CustomLabel type="price" valuePrice={value.OldPrice / 10} valueOldPrice={`${value.Price / 10}تومان`} label="قیمت" />
                                    )}
                                </div>
            ...
            />
            <CustomModal show={showModalEdit}
                onClose={() => {
                    setShowModalEdit((showModal) => !showModal);
                }}
                content={<Edit />}
